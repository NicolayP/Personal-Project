...
static void cdcacm_data_rx_cb(usbd_device *usbd_dev, uint8_t ep __attribute__((unused))) {
...
static void cdcacm_set_config(usbd_device *usbd_dev, uint16_t wValue __attribute__((unused))) {
...
void usb_start( void ) {
	...
	#ifdef STM32F4
		udev = usbd_init(&otghs_usb_driver, &dev, &config,
			usb_strings, 3,
